		...
		if(isset($payload[0])){
			...
			if(isset($userData['user']['email'])) {
				...
				if (isset($result[0])) {
					...
					$usql = "update dna_token_validation set is_enable=?,client_id_test,=?,client_secret_test=?,client_terminal_id_test=?,client_id=?,client_secret=?,client_terminal_id=?,acess_token=? where email_id=? and store_hash=?";
					//echo $usql;exit;
