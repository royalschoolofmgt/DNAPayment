					...
					if(isset($values[$dbv['val']])){
						...
						if($dbv['val'] == "action"){
							...
							if($values['status'] == "CONFIRMED" && $values['type'] == "AUTH" && ($values['settlement_status'] == "PENDING" || $values['settlement_status'] == "FAILED")){
								$actions .= '<a class="btn btn-line" href="cancelOrder.php?bc_email_id='.$_REQUEST['email_id'].'&key='.$_REQUEST['key'].'&auth='.base64_encode(json_encode($values['invoice_id'])).'" ><button type="button" class="btn badges">Cancel</button></a>';
								$actions .= '<a class="btn btn-line" href="settleOrder.php?bc_email_id='.$_REQUEST['email_id'].'&key='.$_REQUEST['key'].'&auth='.base64_encode(json_encode($values['invoice_id'])).'" ><button type="button" class="btn btn-outline-primary">Settle</button></a>';
								...
								if (count($ref_result) > 0) {
									$actions .= '<a class="btn btn-line" style="width: 100%;margin-bottom: 5px;" href="refundOrder.php?bc_email_id='.$_REQUEST['email_id'].'&key='.$_REQUEST['key'].'&auth='.base64_encode(json_encode($values['invoice_id'])).'" ><button type="button" style="width: 75px;margin-left: 5px;" class="btn btn-outline-success">Refunded</button></a>';
								}else{
									$actions .= '<a data-payment-name="'.@$api_response['paymentMethod'].'" class="btn btn-line refundOrderProcess" href="refundOrder.php?bc_email_id='.$_REQUEST['email_id'].'&key='.$_REQUEST['key'].'&auth='.base64_encode(json_encode($values['invoice_id'])).'" ><button type="button" class="btn btn-outline-primary">Refund</button></a>';
								}
							}else if($values['status'] == "CANCELLED"){
								$actions .= '<a class="btn btn-line" style="width: 100%;margin-bottom: 5px;" href="cancelOrder.php?bc_email_id='.$_REQUEST['email_id'].'&key='.$_REQUEST['key'].'&auth='.base64_encode(json_encode($values['invoice_id'])).'" ><button type="button" class="btn badges" style="width: 80px;margin-left: 5px;" >Cancelled</button></a>';
								...
								$ref_result = $ref_stmt->fetchAll();
								if (count($ref_result) > 0) {
									$actions .= '<a class="btn btn-line" style="width: 100%;margin-bottom: 5px;" href="refundOrder.php?bc_email_id='.$_REQUEST['email_id'].'&key='.$_REQUEST['key'].'&auth='.base64_encode(json_encode($values['invoice_id'])).'" ><button type="button" class="btn btn-outline-success" style="width: 75px;margin-left: 5px;" >Refunded</button></a>';
								}else{
									$actions .= '<a data-payment-name="'.@$api_response['paymentMethod'].'" class="btn btn-line refundOrderProcess" href="refundOrder.php?bc_email_id='.$_REQUEST['email_id'].'&key='.$_REQUEST['key'].'&auth='.base64_encode(json_encode($values['invoice_id'])).'" ><button type="button" class="btn btn-outline-primary">Refund</button></a>';
								}
							}
