...
if(!empty($data)){
	...
	if(isset($data['success']) && $data['success']){
		...
		if($settled){
			$usql = 'update order_payment_details set status = ?,settlement_status=?,api_response=?,settlement_response=? where order_id=?';
			$stmt = $conn->prepare($usql);
			$stmt->execute(["CONFIRMED","CHARGE",addslashes(json_encode($data)),addslashes(json_encode($data)),$invoiceId]);
		}else{
		...
		if (isset($result_order_payment[0])) {
			...
			if (isset($result[0])) {
				...
				if($customer_id > 0 && !empty($merchant_card_token)){
					try{
						...
						if (isset($result_card_token[0])) {
							...
							$stmt_card_token_insert->execute([$result_order_payment['email_id'],$result_order_payment['token_validation_id'],$customer_id,$customer_email_id,$merchant_card_token,$merchant_card_name,$merchant_card_schema,$merchant_card_pan,$merchant_card_schema_id,$merchant_card_expiry]);
						}
					}catch(\Exception $e){
				...
				$validCondition = (isset($cartData['cart']['customer_id'],$cartData['isStoreCreditApplied']) && $cartData['cart']['customer_id'] > 0 && ($cartData['isStoreCreditApplied'] === true));
				if ($validCondition) {
					$totalCartPrice     = getCustomerStoreAmount($result_order_payment['email_id'], $result_order_payment['token_validation_id'], $cartData['cart']['customer_id'], $grandTotal);
				}
				$createOrder['customer_message'] = $cartData['customer_message'];
				$createOrder['customer_locale'] = "en";
				$createOrder['total_ex_tax']       = $totalCartPrice-$cartData['tax_total'];
