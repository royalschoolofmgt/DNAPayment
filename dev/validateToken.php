...
if(isset($_REQUEST['client_id']) && isset($_REQUEST['client_secret']) && isset($_REQUEST['client_terminal_id'])){
	...
	if(!empty($email_id) && !empty($key)){
		...
		$is_test_live = $_REQUEST['is_test_live'];
		...
		if (isset($result[0])) {
			...
			if(!empty($_REQUEST['client_id']) && !empty($_REQUEST['client_secret']) && !empty($_REQUEST['client_terminal_id'])){
				$valid = validateToken($email_id,$_REQUEST['client_id'],$_REQUEST['client_secret'],$_REQUEST['client_terminal_id'],$validation_id,$is_test_live);
				if($valid){
					$sellerdb = $result['sellerdb'];
					$data = createFolder($sellerdb,$email_id,$validation_id,$is_test_live);
					if($is_test_live == '1'){
						$sql = 'update dna_token_validation set is_test_live=?,client_id=?,client_secret=?,client_terminal_id=? where email_id=? and validation_id=?';
					}else{
						$sql = 'update dna_token_validation set is_test_live=?,client_id_test=?,client_secret_test=?,client_terminal_id_test=? where email_id=? and validation_id=?';
					}
					//echo $sql;exit;
					$stmt = $conn->prepare($sql);
					$stmt->execute([$is_test_live,$_REQUEST['client_id'],$_REQUEST['client_secret'],$_REQUEST['client_terminal_id'],$email_id,$validation_id]);
...
function createFolder($sellerdb,$email_id,$validation_id,$is_test_live){
	$conn = getConnection();
							...
							success: function (cartres) {
                                ...
                                if (parseFloat(grandTotal) > 0) {
                                    ...
                                    if (parseFloat(StoreCreditAmount) > 0) {
                                        ...
                                        totalCartPrice = parseFloat(grandTotal);
                                    }
                                } else {
                                    totalCartPrice = parseFloat(grandTotal);
                                }
