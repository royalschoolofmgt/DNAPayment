...
<body>
								...
								<ul class="user-detail">
									...
									<li>
										<h5 class="user-head">Client Secret</h5>
											...
											<div class="section-update" id="saveChangesButton" style="display:none;">
												<button type="button" class="btn btn-order" id="saveChanges">Save</button>
											</div>
										...
										<td>
											<?php
												$actions = '';
													...
													}else if($values['status'] == "CONFIRMED" || $values['status'] == "CANCELLED"){
														$ref_stmt = $conn->prepare("SELECT * FROM order_refund where email_id='".$_REQUEST['bc_email_id']."' and invoice_id='".$values['invoice_id']."' and refund_status='REFUND'");
														$ref_stmt->execute();
														$ref_stmt->setFetchMode(PDO::FETCH_ASSOC);
														$ref_result = $ref_stmt->fetchAll();
	...
	$('.btn-toggle').click(function() {
		...
		$('body #tableText').hide();
		$('body #tableText1').hide();
		$('body #tableText2').hide();
		...
		$(document).ready(function() {
			...
			$('body').on('click','#saveChanges',function(e){
				...
				$.ajax({
					...
					data:{client_id:client_id,client_secret:client_secret,client_terminal_id:client_terminal_id,is_test_live:is_test_live},
