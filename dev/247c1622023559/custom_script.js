		...
		var stIntId = setInterval(function() {
			if($(".checkout-step--payment").length > 0) {
				if($("#247dnapayment").length == 0){
					$(".checkout-step--payment .checkout-view-header").after('<div id="247dnapayment" class="checkout-form" style="padding:1px;display:none;"><div id="247Err" style="color:red"></div><form id="dnapaymentForm" name="dnapaymet"><input type="hidden" id="247dnakey" value="eyJlbWFpbF9pZCI6ImJpZ2lAMjQ3Y29tbWVyY2UuY28udWsiLCJrZXkiOiIxIn0=" ><button type="submit" class="button button--action button--large button--slab optimizedCheckout-buttonPrimary" style="background-color: #424242;border-color: #424242;color: #fff;">DEBIT/CREDIT CARDS | powered by DNA PAYMENTS</button></form></div>');
					loadStatus();
												...
												if(res.status){
													if(res.card_token != ""){
														...
														window.DNAPayments.configure ({
															cards: [
																	{
																		merchantTokenId: card_token_data.merchantTokenId,
																		useStoredBillingData: card_token_data.useStoredBillingData,
																		cardName: card_token_data.cardName,
																		isCSCRequired: card_token_data.isCSCRequired
																	}
															]
														});
