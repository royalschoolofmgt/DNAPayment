...
if(isset($postData['invoice_id'])){
	...
	if(isset($result_refund[0]) && ($result_refund[0]['status'] == "CONFIRMED")) {
		...
		$logger->info("Before processCancel API call");
		$res = processCancel($email_id,$request,$validation_id);
		$logger->info("Cancel API response: ".json_encode($res));
		if(isset($res['response']) && isset($res['response']['transactionState']) && ($res['response']['transactionState'] == 'CANCEL')){
			$usql = "UPDATE order_payment_details set status=? where order_id=?";
			$stmt = $conn->prepare($usql);
			$stmt->execute(['CANCELLED',$postData['invoice_id']]);
			$statusResponse = updateOrderStatus($email_id,$postData['invoice_id'],$validation_id);
			header("Location:cancelOrder.php?bc_email_id=".@$_REQUEST['bc_email_id']."&key=".@$_REQUEST['key']."&auth=".base64_encode(json_encode($postData['invoice_id'])).'&error=0');
		}else{
			header("Location:cancelOrder.php?bc_email_id=".@$_REQUEST['bc_email_id']."&key=".@$_REQUEST['key']."&auth=".base64_encode(json_encode($postData['invoice_id'])).'&error=2');
		}
	...
	if (isset($result[0])) {
		...
		//if(!empty($result['client_id']) || !empty($result['client_secret']) || !empty($result['client_terminal_id'])){
			...
			if(isset($order_details['order_id']) && !empty($order_details['order_id'])){
				...
				$staff_comments .= "\n\nPayment Number : ".$invoice_id.",Status : Cancelled,Cacelled Date : ".date("Y-m-d h:i A");
				
				$request_u = array("status_id"=>5,"staff_notes"=>$staff_comments);
				...
				$stmt->execute([$email_id,"BigCommerce","Cancel Order",addslashes($url_u),addslashes($request_u),addslashes($res_u),$validation_id]);
...
function processCancel($email_id,$request,$validation_id){
	$conn = getConnection();
	$data = array();
	...
	if(!empty($bearer_token)){
		...
		if (isset($result[0])) {
			...
			$stmt = $conn->prepare($log_sql);
			$stmt->execute([$email_id,"DNA","Cancel Process",addslashes($url),addslashes($request),addslashes($res),$validation_id]);
